...
class LoanRepository extends ILoanRepository {
  ...
  Future<LoanLoan?> getLoanById(String id) {
    return (db.select(db.loanTable)..where((t) => t.id.equals(id)))
        .getSingleOrNull();
  }
  ...
  Future<void> addLoan({
    ...
    await db.into(db.loanTable).insert(
  ...
  Future<double> getOutstandingAmount(String loanId) async {
    ...
    final repayments = await (db.select(db.loanRepaymentTable)
      ..where((t) => t.loanId.equals(loanId))).get();
    final totalRepaid = repayments.fold<double>(0.0, (sum, r) => sum + r.amount);
